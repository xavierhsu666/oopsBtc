...
INDICATOR_MODE = "muti_Index_Set"  # mode = scalping or muti_Index_Set
TPSL_STRATEGY = "BaseOnATRRatio"  # STRATEGY = BaseOnATRRatio or Based
ACCOUNT_MODE = "DEMO"  # mode = DEMO or LIVE
...
SL_OPTIMIZER_THRESHOLD = 0.2  # ATR - SL優化器 TH(0~1)
MIN_PROFIT = 0.5  # 最小獲利
MAX_LOSS = INITIAL_BALANCE * 0.165  # 最大虧損
PRINT_LOG_BTN = False  # 開啟print log
MAX_SPREAD = 0.2  # scalping spread

TELEGRAM_TOKEN = "8311467265:AAHRI8fd7xHgx4HZH4FEBQ78vCx9wwsc6w0"
CHAT_ID = "1188811502"

if ACCOUNT_MODE == "LIVE":
    account_balance_detail = bn.get_account_balance()
    INITIAL_BALANCE = account_balance_detail["available_balance"]  # 初始資金
...
def calculate_indicators(df):
    ...
    df["EMA9"] = df["close"].ewm(span=4, adjust=False).mean()
    df["EMA21"] = df["close"].ewm(span=8, adjust=False).mean()
...
# ===== 找前一 M15 的高低（改為迭代搜尋直到找到為止）=====
def get_next_m15_high_low_levels(m15_df, latest, direct):
    close = latest["close"]
    take_profit = None  # 始終為 None，直到找到合格的水平
    stop_loss = None  # 始終為 None，直到找到合格的水平

    # 從倒數第二根 K 線開始向後搜尋
    for i in range(1, len(m15_df), 1):
        index = i * -1
        # 確保只使用 current_time 之前的 K 線
        # .iloc[index] 是倒數第 i 根 K 線 (i=1 是前一根)
        ref = m15_df[m15_df["timestamp"] < latest["timestamp"]].iloc[
            index
        ]  # 建議使用 < 避免 current_time 對應的 bar

        # 定義 ATR 條件的最小值
        tp_min_dist = latest["ATR"] * (1 - SL_OPTIMIZER_THRESHOLD)
        sl_min_dist = latest["ATR"] * SL_OPTIMIZER_THRESHOLD

        if direct == "LONG":
            # --- 止盈 (TP) 修正 ---
            # 條件: 尚未找到 TP, 參考高點在 Close 上方, 距離足夠
            if (
                take_profit is None
                and ref["high"] > close
                and abs(ref["high"] - close) > tp_min_dist
            ):
                take_profit = ref["high"]

            # --- 止損 (SL) 修正 ---
            # 條件: 尚未找到 SL, 參考低點在 Close 下方, 距離足夠
            elif (
                stop_loss is None
                and ref["low"] < close
                and abs(close - ref["low"]) > sl_min_dist
            ):
                stop_loss = ref["low"]

        elif direct == "SHORT":
            # --- 止盈 (TP) 修正 ---
            # 條件: 尚未找到 TP, 參考低點在 Close 下方, 距離足夠
            if (
                take_profit is None
                and ref["low"] < close
                and abs(close - ref["low"]) > tp_min_dist
            ):
                take_profit = ref["low"]

            # --- 止損 (SL) 修正 ---
            # 條件: 尚未找到 SL, 參考高點在 Close 上方, 距離足夠
            elif (
                stop_loss is None
                and ref["high"] > close
                and abs(ref["high"] - close) > sl_min_dist
            ):
                stop_loss = ref["high"]
    ...
    if take_profit is None:
        # 設置預設 TP，例如：使用 ATR * 1.5
        if direct == "LONG":
            take_profit = close + latest["ATR"] * (1 - SL_OPTIMIZER_THRESHOLD)
        elif direct == "SHORT":
            take_profit = close - latest["ATR"] * (1 - SL_OPTIMIZER_THRESHOLD)

    if stop_loss is None:
        # 設置預設 SL，例如：使用完整的 ATR
        if direct == "LONG":
            stop_loss = close - latest["ATR"] * (SL_OPTIMIZER_THRESHOLD)
        elif direct == "SHORT":
            stop_loss = close + latest["ATR"] * (SL_OPTIMIZER_THRESHOLD)
...
def switch_TPSL_Strategy(df_ref, latest, direction):
    m15_high, m15_low = get_last_m15_levels(df_ref, latest["timestamp"])
    if TPSL_STRATEGY == "BaseOnATRRatio":
        take_profit, stop_loss = get_next_m15_high_low_levels(df_ref, latest, direction)
    elif TPSL_STRATEGY == "Based":
        if direction == "LONG":
...
def generate_signal(df_main, df_ref, balance):
    ...
    if latest["EMA9"] > latest["EMA21"]:
        if USE_RSI_FILTER and latest["RSI"] >= 70:
            msg = f"LONG RSI 過濾>=70: {latest['RSI']} 不做交易"
            print_log(msg)
            return None
        # if USE_TREND_FILTER and (m15_trend_down or m5_trend_down):
        if USE_TREND_FILTER and (m5_trend_down):
            msg = f"LONG Trend Filter 順勢單: M5Down={m5_trend_down}, M15Down={m15_trend_down} 不做交易"
            print_log(msg)
            return None
        # 根據TPSL策略進行調整
        take_profit, stop_loss = switch_TPSL_Strategy(df_ref, latest, "LONG")
        ...
        if USE_STOP_TAKE_M15 and (
            ...
            msg = f"LONG USE_STOP_TAKE_M15 價格不再區間內 stop_loss-entry-take_profit: {stop_loss:.2F}-{entry_price:.2F}-{take_profit:.2F} 不做交易"
            print_log(msg)
            return None

        if USE_RR_FILTER and rr <= RR_THRESHOLD:
            msg = f"LONG RR 過濾<={RR_THRESHOLD}: {rr:.2f} 不做交易"
            print_log(msg)
            return None
        ...
        if (sch_profit - profit_close_fee - open_fee) <= MIN_PROFIT:
            msg = f"LONG MIN_PROFIT<={MIN_PROFIT}: {(sch_profit - profit_close_fee - open_fee):.2f} 不做交易"
            print_log(msg)
            return None
        if abs(sch_loss - loss_close_fee - open_fee) >= MAX_LOSS:
            msg = f"LONG MAX_LOSS>={MAX_LOSS}: {abs(sch_loss - loss_close_fee - open_fee):.2F} 不做交易"
            print_log(msg)
            return None
    ...
    # 空單
        ...
        if USE_RSI_FILTER and latest["RSI"] <= 30:
            msg = f"SHORT RSI 過濾<=30: {latest['RSI']} 不做交易"
            print_log(msg)
            return None
        # if USE_TREND_FILTER and (m15_trend_up or m5_trend_up):
        if USE_TREND_FILTER and (m5_trend_up):
            msg = f"SHORT Trend Filter 順勢單: M5UP={m5_trend_up}, M15UP={m15_trend_up} 不做交易"
            print_log(msg)
            return None
        entry_price = latest["close"]

        # 根據TPSL策略進行調整
        take_profit, stop_loss = switch_TPSL_Strategy(df_ref, latest, "SHORT")
        if (stop_loss - entry_price) == 0:
            rr = 0
        else:
        ...
        if USE_STOP_TAKE_M15 and (
            ...
            if take_profit >= entry_price:
                msg = f"SHORT USE_STOP_TAKE_M15(TP>=ENTRY) 價格不再區間內 entry-take_profit: {entry_price:.2F}-{take_profit:.2F} 不做交易"
                print_log(msg)
            elif stop_loss <= entry_price:
                msg = f"SHORT USE_STOP_TAKE_M15(SL<=ENTRY) 價格不再區間內 stop_loss-entry: {stop_loss:.2F}-{entry_price:.2F} 不做交易"
                print_log(msg)
        ...
        if USE_RR_FILTER and rr <= RR_THRESHOLD:
            msg = f"SHORT RR 過濾<={RR_THRESHOLD}: {rr:.2f} 不做交易"
            print_log(msg)
            return None
        if (sch_profit - profit_close_fee - open_fee) <= MIN_PROFIT:
            msg = f"SHORT MIN_PROFIT<={MIN_PROFIT}: {(sch_profit - profit_close_fee - open_fee):.2f} 不做交易"
            print_log(msg)
            return None
        if abs(sch_loss - loss_close_fee - open_fee) >= MAX_LOSS:
            msg = f"SHORT MAX_LOSS>={MAX_LOSS}: {abs(sch_loss - loss_close_fee - open_fee):.2F} 不做交易"
            print_log(msg)
            return None
        return {
            "signal": "SHORT",
...
def notify_startup():
    ...
    message = (
        ...
        f"--- <b>核心配置</b> ---\n"
        f"* <b>帳戶模式:</b> <code>{ACCOUNT_MODE}</code>\n"
        ...
        f"--- <b>策略</b> ---\n"
        f"* <b>TPSL策略:</b> {TPSL_STRATEGY}\n"
        f"* <b>指標模式:</b> {INDICATOR_MODE}\n\n"
        f"--- <b>策略參數</b> ---\n"
        f"* <b>SL優化器-TH:</b> {SL_OPTIMIZER_THRESHOLD}\n"
        f"* <b>Scraping Spread:</b> {MAX_SPREAD}\n"
        f"* <b>最大損失:</b> {MAX_LOSS}\n"
        f"* <b>最小獲利:</b> {MIN_PROFIT}\n\n"
        f"--- <b>策略濾網</b> ---\n"
...
def backtest():
    ...
    for i in range(21, len(df_main_b)):
        ...
        # ===== 開倉條件 =====
        if position == 0:
            signal_info = switch_indicator_mode(df_main, df_ref, balance)
            if signal_info:
...
def optimize_parameters():
    ...
    for idx, params in enumerate(param_combinations, 1):
        ...
        global USE_RR_FILTER, USE_STOP_TAKE_M15, USE_SL_OPTIMIZER, SL_OPTIMIZER_THRESHOLD
...
def switch_indicator_mode(df_main, df_ret, balance):
    if INDICATOR_MODE == "muti_Index_Set":
        info = generate_signal(df_main, df_ret, balance)
    elif INDICATOR_MODE == "scalping":
        info = generate_scalping_signal(df_main, balance)
    return info
